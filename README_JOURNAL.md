...
| 56 | manuscript-to-JSON, primary-citation, and journal-only red-marking consistency audit | `outputs/artifact_consistency_audit.json` |
...
| 62 | aggregate the one-seed 50/75/100% trained-row boundary | `outputs/data_side/stip_independent_embedding_frequency_phase_sweep_20260802.json` |
...
SEEDS="20260802" FRACTIONS="0.50 0.75 1.0" bash 60b_run_stip_independent_grid.sh
python3 62_aggregate_stip_fraction_sweep.py --input /home/user/p050_journal_targets/results/stip_independent_embedding_frequency_f050_seed20260802.json --input /home/user/p050_journal_targets/results/stip_independent_embedding_frequency_f075_seed20260802.json --input /home/user/p050_journal_targets/results/stip_independent_embedding_frequency_f10_seed20260802.json --output /home/user/p050_journal_targets/results/stip_independent_embedding_frequency_phase_sweep_20260802.json
```
...
Scripts 60--62 train three separate GPT-2 partial-embedding checkpoints on 500,000 SciFact/NFCorpus tokens.  The transformer is frozen; only the most frequent 25% of active tied embedding rows receive gradients for 200 steps.  Across seeds 20260802--20260804, all 13,285 selected rows change and zero unselected rows change.  Search over the actually changed rows gives mean/minimum Top-1 96.8537%/96.7503%, mean Top-5 97.1852%, and mean sufficient-certificate rate 32.2079%; mean validation loss falls from 10.2809 to 9.1568.  A one-seed boundary sweep then updates 50%, 75%, and 100% of active rows (8,886--17,772 rows) and retains 96.7064%--96.8265% Top-1 without changing any unselected row.  No failure transition appears, so only the 25% condition is repeated across three seeds.  Documents are tokenized to an ID stream before 64-token model-input blocks are formed; the tokenizer max-length warning in stdout refers to an unchunked source document, not to a model input.  The six checkpoints remain server-side under `/home/user/p050_journal_targets/checkpoints`; their SHA-256 values, run parameters, per-run results, JSONs, and stdout logs are retained without committing model weights.
