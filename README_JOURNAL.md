...
Script 43 computes every public signature's exact nearest-neighbor separation and applies the sufficient half-margin certificate.  Certified rates are 73.5539% (GPT-2 word), 97.4969% (GPT-2 position zero), 99.9115% (BERT word), and 99.9869% (BERT position zero).  The token-ID reindexing control changes no rate, and all four private signature dictionaries have zero bitwise row collisions.  The partial-row-update campaign applies the already measured full-fine-tune outcome to random subsets over 20 repetitions; it does not train additional checkpoints.  At 75% updated rows, mean top-1 is 97.3296% for GPT-2 word rows and 99.9379% for BERT word rows.  The vocabulary-overlap control counts unaligned rows as semantic failures and preserves only pseudotoken linkage.  Full per-token arrays remain server-side in `/home/user/p050_journal_targets/results/stip_margin_detail_exact_rows_20260802` (3.1 MiB); the compact JSON and log are copied locally.
