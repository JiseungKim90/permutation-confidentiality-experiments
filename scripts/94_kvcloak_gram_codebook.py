...
def search(bank: np.ndarray, queries: np.ndarray, chunk: int = 32):
    ...
    for start in range(0, queries.shape[0], chunk):
        similarities = np.clip(queries[start : start + chunk] @ transposed, -1.0, 1.0)
        top_two = np.argpartition(-similarities, kth=1, axis=1)[:, :2]
        values = np.take_along_axis(similarities, top_two, axis=1)
        order = np.argsort(-values, axis=1)
        top_two = np.take_along_axis(top_two, order, axis=1)
        values = np.take_along_axis(values, order, axis=1)
        predictions.extend(int(value) for value in top_two[:, 0])
        best_distances.extend(float(value) for value in np.sqrt(np.maximum(0.0, 2.0 - 2.0 * values[:, 0])))
        second_distances.extend(float(value) for value in np.sqrt(np.maximum(0.0, 2.0 - 2.0 * values[:, 1])))
...
def main() -> None:
    ...
    result = {
        ...
        "prespecified_followup_hypothesis": "a normalized Gram sketch identifies enrolled blocks at b=d under official reuse and refreshed orthogonal left transforms",
