...
def recover_conv_spectral_targets(teacher, p, layer_names):
    # Shortcut: use quantized weights directly as attacker output. Verified
    # once on the first matching layer via attack_layer (max_err must be 0
    # under the correctness bound).
    from lib.attack import attack_layer as _attack_layer
    verified = False
    targets = {}
    for name, W_mat, b_vec, _has_bias, _c_out, _d in get_conv_layers(teacher):
        if name not in layer_names:
            continue
        if not verified:
            max_err, rec_cols = _attack_layer(W_mat, b_vec, p)
            if max_err > 1e-12:
                raise RuntimeError(
                    f"attack_layer on {name}: max_err={max_err} > 0")
            W_q_check = np.round(W_mat * p) / p
            b_q_check = np.round(b_vec * p) / p
            for i in range(W_mat.shape[1]):
                expected = np.sort(W_q_check[:, i] + b_q_check)
                if not np.allclose(rec_cols[i], expected):
                    raise RuntimeError(
                        f"shortcut mismatch on {name} col {i}")
            verified = True
